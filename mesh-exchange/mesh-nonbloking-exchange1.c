//from sc17 ken
//It's a 4X4 mesh. processes commnicate with their neighbors using nonblocking way.
...
int main(int argc, char* argv[])
{
    ...
    for (j = 0; j < LOOP_NUM; j++) {
      ...
      if (down != -1) {
          ...
          MPI_Send(x, DATA_NUM, MPI_INT, down, 0, MPI_COMM_WORLD);
      }

      if (right != -1) {
          /* If right neighbor exists, send a message */
          MPI_Send(x, DATA_NUM, MPI_INT, right, 0, MPI_COMM_WORLD);
      }

      if (up != -1) {
          /* If up neighbor exists, send a message */
          MPI_Send(x, DATA_NUM, MPI_INT, up, 0, MPI_COMM_WORLD);
      }

      if (left != -1) {
          /* If left neighbor exists, send a message */
          MPI_Send(x, DATA_NUM, MPI_INT, left, 0, MPI_COMM_WORLD);
      }
